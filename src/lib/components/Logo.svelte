<script>
    ...
    // Props for size control if needed
    export let className = "";
</script>

<div class={`relative ${className} {$isMenuOpen ? 'logo-particle-mode' : ''}`}>
    {#if $isMenuOpen}
        <!-- Particle Effects Layer (only visible when menu is open) -->
        <div class="absolute inset-0 z-0 pointer-events-none">
            {#each Array(20) as _, i}
                <div
                    class="absolute w-1 h-1 bg-red-500 rounded-full animate-particle"
                    style="
